  ...
  parallel(
    ...
    'UWP': {
      nodeWithCleanup('windows') {
        ...
        bat """
          "${windowsNugetCmd}" restore Realm.sln
          "${tool 'msbuild'}" Tests/Tests.UWP/Tests.UWP.csproj /p:Configuration=${configuration} /p:SolutionDir="${env.WORKSPACE}\\" /p:UseDotNetNativeToolchain=false /p:AppxBundlePlatforms="x86|x64|ARM" /p:AppxBundle=Always
        """
